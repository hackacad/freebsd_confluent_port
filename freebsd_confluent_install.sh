...
echo fdesc   /dev/fd         fdescfs         rw      0       0 >> test.txt
echo proc    /proc           procfs          rw      0       0 >> test.txt
echo lineprocfs	/compat/linux/proc	linprocfs	rw	0	0 >> test.txt
echo linsysfs		/compat/linux/sys	linsysfs		rw	0	0 >> test.txt
echo tmpfs		/compat/linux/dev/shm	tmpfs	rw,mode=1777		0	0 >>test.txt
mount -a
...
brandelf -t Linux /home/confluent-7.0.0/libexec/cli/linux_amd64/confluent 
brandelf -t Linux /home/confluent-7.0.0/bin/*
cd && touch .testrc
echo PATH=/sbin:/bin:/usr/sbin:/usr/bin:/usr/local/sbin:/usr/local/bin:~/bin:/home/confluent-7.0.1/libexec/cli/linux_amd64/ >> /.testrc
echo export PATH >> /.testrc
echo CONFLUENT_HOME=/home/confluent-7.0.1 >> /.testrc
echo export CONFLUENT_HOME >>/.testrc
